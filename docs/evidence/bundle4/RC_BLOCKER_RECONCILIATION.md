...
| 4 — clean reproducibility | K | **PASS** (floor) · 3.11+ residual = environment dependency | `RC_K_CLEAN_REBUILD.md`: isolated locks-only venv, package builds/installs, site-packages import with no PYTHONPATH, CLI resolves, full regression 866/0/2 on 3.9.6. No dependency/config defect. 3.11+ recommended-target run needs an operator-provided interpreter (not a candidate requirement; `requires-python>=3.9`, no `>=3.10`-only syntax). |
...
| K | Configuration / Dependencies | **PASS** (clean rebuild on floor; 3.11+ verification is an environment dependency, not a candidate defect) |
