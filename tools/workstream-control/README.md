...
2. Click `LAUNCH` (the prompt is copied to the clipboard).
3. Switch to ChatGPT yourself.
4. Press `Cmd+V`.
5. Press `Enter`.
...
Retrieves the canonical prompt → copies it to the clipboard → marks the workstream `ACTIVE` → persists state → shows **PROMPT COPIED / SWITCH TO CHATGPT / PRESS CMD+V / PRESS ENTER**.

It does **not** open ChatGPT, a browser, or a tab, and does not automate ChatGPT's page. You switch to ChatGPT yourself, paste, and press Enter.
